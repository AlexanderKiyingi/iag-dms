// Package outbox implements the transactional outbox pattern for DMS events.
package outbox
...
// Store wraps the dms_event_outbox table.
type Store struct {
	pool *pgxpool.Pool
}
...
func (s *Store) Enqueue(ctx context.Context, eventType, key string, payload any) error {
	...
	_, err = s.pool.Exec(ctx, `
		INSERT INTO dms_event_outbox (event_type, event_key, payload)
		VALUES ($1, $2, $3::jsonb)
...
func (s *Store) ClaimBatch(ctx context.Context, limit int, backoff time.Duration) ([]Row, error) {
	...
	rows, err := s.pool.Query(ctx, `
		WITH due AS (
			SELECT id FROM dms_event_outbox
			WHERE dispatched_at IS NULL AND available_at <= NOW()
			ORDER BY id
			FOR UPDATE SKIP LOCKED
			LIMIT $1
		)
		UPDATE dms_event_outbox o
